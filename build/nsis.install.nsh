Name "prlx ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install prlx binary
Section "Prlx" PRLX_IDX
  SetOutPath $INSTDIR
  file {{.Prlx}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\prlx.exe"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\prlx.exe" "attach"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${PRLX_IDX}
    SectionGetSize ${PRLX_IDX} $0
