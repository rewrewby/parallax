...
// faucet is an Laxes faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Parallax protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Laxes to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Parallax and the faucet")

	twitterTokenFlag   = flag.String("twitter.token", "", "Bearer token to authenticate with the v2 Twitter API")
	twitterTokenV1Flag = flag.String("twitter.token.v1", "", "Bearer token to authenticate with the v1.1 Twitter API")
)

var laxes = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Laxes", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Parallax address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Parallax light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Parallax protocol stack
	client *prlclient.Client   // Client connection to the Parallax chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Parallax light client protocol
	cfg := prlconfig.Defaults
	...
	// Assemble the prlstats monitoring and reporting service'
	if stats != "" {
		if err := prlstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := prlclient.NewClient(api)
...
// close terminates the Parallax connection and tears down the faucet.
...
// apiHandler handles requests for Laxes grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]any{
		"funds":    new(big.Int).Div(balance, laxes),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Parallax address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/microstack-tech/parallax/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), laxes)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, laxes)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Parallax address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Parallax address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Parallax address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Parallax address to fund on
// success.
...
// returning the username, avatar URL and Parallax address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Parallax address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Parallax address,
