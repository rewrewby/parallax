...
cat <<EOF
## PVM state transition tool

The \`pvm t8n\` tool is a stateless state transition utility. It is a utility
which can
...
$(./pvm t8n -h | grep "trace\|output\|state\.")
...
#### PVM-based errors (\`2\` to \`9\`)

- Other PVM error. Exit code \`2\`
...
./pvm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --state.fork=Frontier+1346 2>/dev/null
if [ $? != 3 ]; then
...
EOF
cmd="./pvm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json"
tick
echo "$cmd"
tick
$cmd 2>/dev/null
...
cmd="./pvm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --output.result=stdout --output.alloc=stdout"
tick
echo "$cmd"
tick
...
cmd="./pvm t8n --input.alloc=./testdata/5/alloc.json -input.txs=./testdata/5/txs.json --input.env=./testdata/5/env.json  --output.alloc=stdout --state.reward=0x80"
output=$($cmd 2>/dev/null)
echo "Output:"
echo "${ticks}json"
echo "$output"
...
cmd="./pvm t8n --state.fork=Frontier+1344 --input.pre=./testdata/1/pre.json --input.txs=./testdata/1/txs.json --input.env=/testdata/1/env.json"
tick
echo "$cmd"
tick
echo ""
...
demo "./pvm --verbosity=1 t8n --input.alloc=./testdata/3/alloc.json --input.txs=./testdata/3/txs.json --input.env=./testdata/3/env.json  --trace"
...
cmd="./pvm t8n --input.alloc=./testdata/4/alloc.json --input.txs=./testdata/4/txs.json --input.env=./testdata/4/env.json  --trace"
tick && echo $cmd && $cmd
errc=$?
tick
...
cmd1="./pvm t8n --input.alloc=./testdata/1/alloc.json --input.txs=./testdata/1/txs.json --input.env=./testdata/1/env.json --output.alloc=stdout"
cmd2="./pvm t8n --input.alloc=stdin --input.env=./testdata/1/env.json --input.txs=./testdata/1/txs.json"
echo "$ticks"
echo "$cmd1 | $cmd2"
output=$($cmd1 | $cmd2)
echo $output
...
echo "actual blocknumber (exposed to the PVM) would not increase."
echo ""
...
echo "to use the pvm to go from \`json\` input to \`rlp\` input."
echo ""
...
demo "./pvm t8n --state.fork=London --input.alloc=./testdata/13/alloc.json --input.txs=./testdata/13/txs.json --input.env=./testdata/13/env.json --output.result=alloc_jsontx.json --output.body=signed_txs.rlp"
...
demo "./pvm t8n --state.fork=London --input.alloc=./testdata/13/alloc.json --input.txs=./signed_txs.rlp --input.env=./testdata/13/env.json --output.result=alloc_rlptx.json"
