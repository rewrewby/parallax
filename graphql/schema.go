...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Parallax address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Parallax account at a particular block.
    type Account {
    ...
    type Block {
        ...
        # Account fetches an Parallax account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Parallax account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Parallax block by number or by hash. If neither is
